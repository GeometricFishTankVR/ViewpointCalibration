...
public static class CubeeMath
{
    ...
    public static Vector3 SphericalToCartesian(Vector3 spherical)
    {
        ...
        return new Vector3(Mathf.Cos(theta) * Mathf.Sin(phi), Mathf.Cos(phi), Mathf.Sin(theta)*Mathf.Sin(phi)) * radius;
    }

    public static Vector3 CartesianToSpherical(Vector3 cartesian)
    {
        float radius = cartesian.magnitude;
        float theta = Mathf.Atan(cartesian.z / cartesian.x);
