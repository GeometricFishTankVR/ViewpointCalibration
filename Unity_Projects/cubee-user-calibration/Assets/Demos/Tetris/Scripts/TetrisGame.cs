...
public class TetrisGame : MonoBehaviour
{
    ...
    [Tooltip( "The number of blocks around the equator of the sphere." )]
    ...
    [Reorderable, Tooltip( "The collection of piece prefabs." )]
    public TetrisPieceSet PieceCollection;

    [Reorderable]
    public LayerColorList LayerColors;
    ...
    [System.Serializable]
    public class LayerColorList : ReorderableArray<Color>
    {
    ...
    [System.Serializable]
    public class TetrisPieceSet : ReorderableArray<TetrisPiece>
    {
        private IList<int> DistributedList;

        private void ComputeDistribedList()
        {
            DistributedList = new List<int>();
            for( int i = 0; i < Length; i++ )
            {
                for( int c = 0; c < this[i].Distribution; c++ )
                    DistributedList.Add( i );
            }
        }
        ...
        {
            if( DistributedList == null ) ComputeDistribedList();
            var idx = Random.Range( 0, DistributedList.Count );
            return DistributedList[idx];
        }
        ...
        public TetrisPiece GetRandomPiece()
        {
            if( DistributedList == null ) ComputeDistribedList();
            var idx = Random.Range( 0, DistributedList.Count );
            return this[DistributedList[idx]];
        }
    }
    ...
    void Start()
    {
        ...
        ClearCondition = new RingClearCondition();
        ...
        {
            for( int z = 0; z < BlocksVertical; z++ )
            {
                // 
        ...
        // 
        StartingX = Random.Range( 0, BlocksHorizontal );
    ...
    void UpdateInput()
    {
        ...
        // X Motion
        if( Input.GetButtonDown( "MoveRight" ) ) position.x++;
        if( Input.GetButtonDown( "MoveLeft" ) ) position.x--;

        // Z Motion
        if( Input.GetButtonDown( "MoveUp" ) ) position.z++;
        if( Input.GetButtonDown( "MoveDown" ) ) position.z--;
    ...
    void UpdateTick()
    {
        ...
        else
        {
            ...
            if( IsValidToPlace( ActivePiece ) )
            {
                ...
                StartingX = (int) ActivePiece.transform.position.x;

                // Determine clear condition
                var clusterCount = ClearCondition.ProcessClusters( this );
                // TODO: Add score with clusterCount 
    ...
    {
        if( cancel ) CancelInvoke( "UpdateTick" );
    ...
    void SpawnTetramino()
    {
        ...
        var ActivePiece_Transform = ActivePiece.GetComponent<Transform>();
        ActivePiece_Transform.position = new Vector3( StartingX, BlockLayers, startingZ );
    ...
    void UpdateGhost()
    {
        ...
        foreach( var child in GhostPiece.GetChildren() )
        {
            ...
            child_MeshRenderer.material.SetColor( "_Color", LayerColors.GetLayerColor( y ) );
#endif
        }
    }
...
#if USE_COLOR_HINT
            var color = LayerColors.GetLayerColor( y );
            material.SetColor( "_Color", color );
#endif
        }
    ...
    void MoveBackIntoField( TetrisPiece obj )
    {
        ...
        foreach( var child in obj.GetChildren() )
        {
            ...
            {
                if( z >= BlocksVertical ) zShift = Mathf.Max( zShift, z - BlocksVertical + 1 );
                if( z < 0 ) zShift = Mathf.Min( zShift, z );
                if( y < 0 ) yShift = Mathf.Min( yShift, y );
            }
        }
    ...
    internal void ClearCell( int x, int y, int z )
    {
        ...
        // Shift column
        for( int _y = y; _y < BlockLayers - 1; _y++ )
        {
            ...
            if( block != null )
            {
                ...
                var color = LayerColors.GetLayerColor( _y - 1 );
        ...
        StageData[x, BlockLayers - 1, z].Empty();
    }
    ...
    public bool IsValidCoordinate( int x, int y, int z )
    {
        ...
        // Upper Bound
        if( x >= BlocksHorizontal ) return false;
        if( y >= BlockLayers ) return false;
        if( z >= BlocksVertical ) return false;
    ...
    bool IsOverlapField( TetrisPiece piece )
    {
        ...
        foreach( var child in piece.GetChildren() )
        {
            int x, y, z;
            GetDataCoordinate( child.position, out x, out y, out z );
            if( IsCellOccupied( x, y, z ) ) return true;
        }
    ...
    {
        if( IsValidCoordinate( x, y, z ) ) return StageData[x, y, z].Occupied;
        else return y < 0;
    }
    ...
    {
        if( IsValidCoordinate( x, y, z ) ) return StageData[x, y, z].Block;
        else return null;
    }
    ...
    static IEnumerable<Transform> EnumerateChildren( GameObject obj )
    {
        ...
        for( int i = 0; i < piece_Transform.childCount; i++ )
    ...
    {
        obj.hideFlags = HideFlags.HideInHierarchy | HideFlags.HideInInspector | HideFlags.NotEditable | HideFlags.DontSave;
    }
