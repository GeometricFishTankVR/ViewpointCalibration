...
public class FishBehaviour : MonoBehaviour
{
    ...
    private bool IsCloseEnough { get { return Vector3.Distance( transform.position, TargetPosition ) < 1F; } }

    private float ChillTimer = 0F;

    private Vector3 WanderDistribution = Vector3.Normalize( new Vector3( 2, 1, 2 ) );
    ...
    void ChooseNewPlaceToChill()
    {
        ...
        TargetPosition = WanderingBounds.GetRandomLocationInBounds( WanderDistribution );
    ...
    void Update()
    {
        if( IsCloseEnough )
        {
            ...
            if( ChillTimer <= 0 ) ChooseNewPlaceToChill();
            else ChillTimer -= Time.deltaTime; // Count down
        }
        else
        {
